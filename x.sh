#!/usr/bin/env bash
# =============================================================================
# fix-auth-context-only.sh
#
# PROBLEMA EXACTO (confirmado con logs):
#   • dashboard-front /login llama POST /auth/firebase-sso → JWT ✓
#   • guarda JWT en localStorage ✓
#   • redirige a /dashboard ✓
#   • AuthContext (código viejo) usa onAuthStateChanged → fbUser=null
#   • isAuthenticated = !!firebaseUser && !!user = false → back to /login ✗
#
# SOLUCIÓN: reemplazar auth-context.tsx para que use el JWT del localStorage
#           en lugar de Firebase client SDK.
#
# UN SOLO ARCHIVO. Nada más.
#
# USO: ejecutar en el root de real-dashboard-front → commit → deploy en Railway
# =============================================================================
set -euo pipefail
...
step "Sobreescribiendo features/auth/context/auth-context.tsx"
...
echo ""
echo -e "${GREEN}══════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  Listo. Un solo archivo modificado.${NC}"
echo -e "${GREEN}══════════════════════════════════════════════════════${NC}"
echo ""
echo "  Próximos pasos:"
echo "  1. git add features/auth/context/auth-context.tsx"
echo "  2. git commit -m 'fix: auth context JWT-based, sin Firebase client'"
echo "  3. git push → Railway redeploya automáticamente"
echo ""
echo "  Flujo que va a funcionar:"
echo "  real-front → POST firebase-sso → JWT → redirect /auth/sso?token=JWT"
echo "  /auth/sso guarda JWT en localStorage → redirect /dashboard"
echo "  AuthContext lee JWT → GET /auth/me → isAuthenticated=true → dashboard ✓"
echo ""